"""Real-time emotion recognition using HybridEmotionNet.

Runs MediaPipe FaceMesh on each webcam frame to extract 3D landmark coordinates
and a face crop, feeds both into the hybrid model, and renders the predicted
emotion with a probability bar overlay.
"""
...
class HybridInference:
    """Real-time inference engine for HybridEmotionNet.

    Processes individual video frames end-to-end: face detection via MediaPipe,
    coordinate extraction and normalisation, face crop preparation, model
    inference, and temporal smoothing.

    Args:
        model_path: Path to a saved checkpoint (.pth). Defaults to
    ...
    def process_frame(self, frame: np.ndarray):
        ...
        Detects the face, extracts landmarks and a normalised crop, runs the
        hybrid model, applies EMA + sliding-window temporal smoothing, and
        renders the prediction overlay onto the frame in place.

        Args:
            frame: BGR image array as returned by ``cv2.VideoCapture.read()``.

        Returns:
            Tuple of (annotated_frame, emotion_str, confidence_float).
            Returns (frame, None, None) when no face is detected.
        """
        h, w = frame.shape[:2]
        ...
        too_small = face_w < 60 or face_h < 60
        ...
        pad_w = int(face_w * pad_ratio)
        pad_h = int(face_h * pad_ratio)
        ...
        if too_small:
            cv2.putText(
                frame, "Move closer for better accuracy",
                (x_min, y_max + 20),
    ...
    def run(self, camera_index: int = 0):
        """Start the live webcam inference loop.
